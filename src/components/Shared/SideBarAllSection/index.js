...
export default function SideBarAllSection(props) {
  ...
  const handleCategoryFilter = (depth0, depth1, depth2, depth3, depth4) => {
    handleCategoryClicked &&
      handleCategoryClicked(depth0, depth1, depth2, depth3, depth4);
    ...
    setDepths((t) => {
      if (JSON.stringify(t) === JSON.stringify(tt)) return {};
      else return tt;
    });
  };
  ...
  const returnDepth4Instances = (
    depth0Label,
    depth1Label,
    depth2Label,
    depth3Label
  ) => {
    let categoryFilter4 = categories.filter(
      (t) =>
        t.depth0 === depth0Label &&
        t.depth1 === depth1Label &&
        t.depth2 === depth2Label &&
        t.depth3 === depth3Label
    );
    let depth4All = categoryFilter4.map((t) => t.depth4);
    let depth4 = depth4All.filter((t, i) => depth4All.indexOf(t) === i);

    return depth4.map((t, idx) =>
      t === "" ? (
        <></>
      ) : (
        <PlaneAccordion
  ...
  const returnDepth3Instances = (depth0Label, depth1Label, depth2Label) => {
    let categoryFilter3 = categories.filter(
      (t) =>
        t.depth0 === depth0Label &&
        t.depth1 === depth1Label &&
        t.depth2 === depth2Label
    ...
    return depth3.map((t, idx) =>
      t === "" ? (
        ...
        <PlaneAccordion
          ...
          depth={3}
          title={t}
          selected={depths.depth3 === t}
          count={countFilter(depth0Label, depth1Label, depth2Label, t)}
          handleClick={() =>
            handleCategoryFilter(depth0Label, depth1Label, depth2Label, t)
          }
        >
  ...
  const returnDepth2Instances = (depth0Label, depth1Label) => {
    let categoryFilter2 = categories.filter(
      (t) => t.depth0 === depth0Label && t.depth1 === depth1Label
    );
    let depth2All = categoryFilter2.map((t) => t.depth2);
    let depth2 = depth2All.filter((t, i) => depth2All.indexOf(t) === i);

    return depth2.map((t, idx) =>
      t === "" ? (
        <></>
      ) : (
        <PlaneAccordion
    ...
    <div>
      <SideBarAllSectionContainer>
        ...
        {address !== null && address !== undefined && address !== "0x0" ? (
          ...
          <div className="no-wallet-attached">{t("No wallet attached")}</div>
        )}
      </SideBarAllSectionContainer>
    </div>
  );
