...
const MakeOfferModal = (props) => {
  ...
  const priceTypeList = [
    ...
    // { id: 2, name: 'DEXO', key: 'DEXO', icon: theme.images.chainTokenIcon },
  ];
  ...
  useEffect(() => {
    ...
    if (nftItem.collectionAddress !== undefined) {
      invokeServer(
        "get",
        `/api/nft/owner?collectionAddress=${nftItem.collectionAddress.toLowerCase()}&tokenId=${
          nftItem.tokenId
        }`
      )
        .then((res) => {
  ...
  useEffect(() => {
    if (ownerInfo !== undefined) {
      setOwnerList((t) => {
        return offerInfo.owners
          .map((owner, index) => {
            let titem = ownerInfo.filter(
              (tf) => tf.owner.toLowerCase() === owner.owner.toLowerCase()
            );
            return {
              id: index,
              ...
              name:
                titem.length > 0
                  ? `${titem[0].name} - ${owner.balanceLeft}`
                  : owner.balanceLeft.toString(),
            };
          })
  ...
  const commitOffer = async () => {
    ...
    try {
      ...
      let ta = ownerInfo.find(
        (t) => t.address.toLowerCase() === tt.address.toLowerCase()
      );
      ...
      if (tx !== undefined) {
        ...
        let basePrice = tokenAmountWithDecimal(
          parseInt(retval.ti.payment),
          retval.ti.basePrice
        ).toString();
        let saleId = parseInt(retval.saleId);
        let sellerProfile = ownerInfo.find(
          (t) => t.address.toLowerCase() === retval.ti.seller.toLowerCase()
        );
        ...
        <MakeOfferModalDetailContainer>
          ...
          <MakeOfferDetailContainer>
            ...
            <div className="price-description">
              <p>
                {t("Total amount offered:")} {price} {selectedPriceType.name} ={" "}
                {convertPrice && convertPrice(selectedPriceType.id, price)}{" "}
                {t("USD")}
              </p>
            </div>
            ...
            <div className="temrs-container">
              <CheckBox
                id={1}
                label={
                  "By checking this box, I agree to DEXO’s Terms of service"
                }
