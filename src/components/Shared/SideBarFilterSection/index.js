...
export default function SideBarFilterSection(props) {
  const { position, closeSideMenu, listType, filterParams, setFilterParams } =
    props;
  ...
  const [currencyIds, setCurrencyIds] = useState(
    filterParams?.currency?.map((t) => t.id) || []
  );
  const [statusIds, setStatusIds] = useState(
    filterParams?.status?.map((t) => t.id) || []
  );
  const [rarityIds, setRarityIds] = useState(
    filterParams?.rarity?.map((t) => t.id) || []
  );

  const viewTypeList = [
    // {id: 0, name: 'Binance Token (BNB)', key: 'BNB', icon: theme.images.binanceTokenIcon},
    { id: 1, name: "USD", key: "$", icon: theme.images.binanceUsdIcon },
    // {id: 2, name: 'BSC Chain Token (DEXO)', key: 'DEXO', icon: theme.images.chainTokenIcon},
  ];

  const [priceUnit, setPriceUnit] = useState(
    viewTypeList.filter((t) => t.id === filterParams?.price?.unit)
  );
  ...
  const [sortType, setSortType] = useState(
    filterParams?.sort || sortTypeList[0]
  );
  ...
  const handlePriceMinMaxChanged = useCallback(
    ({ min, max }) => {
  ...
  const handleMintIDMinMaxChanged = useCallback(
    ({ min, max }) => {
    ...
    <div>
      <SideBarFilterSectionContainer>
        <div
          ...
          style={{
            justifyContent: listType !== "event" ? "space-between" : "center",
          }}
        ...
        <FilterContent>
          <Accordion title={listType === "event" ? "Event Type" : "Sort By"}>
            <div className="sort-div">
              <IconDropDown
                viewTypeList={sortTypeList}
                initialType={filterParams?.sort?.key || sortTypeList[0].key}
            ...
            <div className="collection-div">
              {defaultCurrencyList.map((cl) => (
                <CheckBox
                  ...
                  onChange={(val) =>
                    handleChangeIds(val, currencyIds, setCurrencyIds)
                  }
                />
              ))}
            </div>
              ...
              <div className="collection-div">
                {defaultStatusList.map((st) => (
                  <CheckBox
                    ...
                    onChange={(val) =>
                      handleChangeIds(val, statusIds, setStatusIds)
                    }
                  />
                ))}
              </div>
            ...
            <div className="rarity-div">
              {defaultRarityList.map((rarity) => (
                <CheckBox
                  ...
                  onChange={(val) =>
                    handleChangeIds(val, rarityIds, setRarityIds)
                  }
                />
              ))}
            </div>
