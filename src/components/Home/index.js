...
export default function Home(props) {
  ...
  const { convertPrice } = useContext(ContractContext);

  const { invokeServer, reloadSaleCounter, reloadSales } =
  ...
  useEffect(() => {
    ...
    } else if (router.pathname.includes("/offer")) {
  ...
  useEffect(() => {
    ...
    if (sales?.length > 0) {
      let priceUSDList = sales.map((t) => {
        let priceUSD;
        //  = parseFloat(convertPrice(t.payment, t.price));
        return priceUSD;
      });
