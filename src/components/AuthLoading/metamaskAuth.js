...
export default function MetamaskAuth({ open, setOpen }) {
  const { t } = useTranslation();
  return (
    <>
      {open && (
        <div className="OverlayReactModal" onClick={() => setOpen(false)}>
          <div
            id="auth-modal"
            className="Content"
            role="dialog"
          ...
          >
            <div className="container">
              <div className="text-center">
                <h2 className="heading">{t("Metamask connection failed")}</h2>

                <p className="paragraph">
                  {t(
                    "Please Install Metamask chrome extension in your Browser to use this application."
                  )}
                </p>
              </div>
            </div>
