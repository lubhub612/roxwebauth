...
const ContractProvider = ({ children }) => {
  ...
  console.log("🚀 ~ file: index.js:21 ~ ContractProvider ~ wallet:", wallet);
  ...
  const convertPrice = useCallback(
    (tokenIndex, tokenAmount) => {
      if (typeof tokenIndex === "string") tokenIndex = parseInt(tokenIndex);

      let paymentItem = payment.find((item) => item.id === tokenIndex);
      if (paymentItem === undefined) return null;

      let priceItem = priceList.find((item) => item.id === tokenIndex);
      if (priceItem === undefined) return null;

      return BigNumber(tokenAmount)
        .times(BigNumber(priceItem.ratio))
        .dp(3)
        .toString();
    },
    [payment, priceList]
  ...
  const createNewCollection = async (t) => {
    ...
    try {
      ...
      if (factoryABI === null || factoryAddress === null) return tx;
  ...
  const createNFT = async (t) => {
    ...
    try {
      ...
      await contract.methods
        .create(wallet?.address, window.web3.utils.toBN(t.supply), t.uri, "0x0")
        .send({ from: wallet?.address })
        .on("receipt", (receipt) => {
          tx = receipt;
  ...
  const getNFTCount = useCallback(
    async (t) => {
      ...
      if (
        t.account === undefined ||
        t.account === "" ||
        t.collectionAddress === undefined ||
        t.colllectionAddress === "" ||
        t.tokenId === undefined ||
        multipleCollectionABI === null
      )
        return null;
      ...
      try {
        ...
        tx = await contract.methods
          .balanceOf(t.account, window.web3.utils.toBN(t.tokenId))
          .call({ from: t.caller });
      } catch (err) {
        console.log(err);
      }
  ...
  const getDefaultFee = useCallback(async () => {
    ...
    try {
      ...
      let v1 = await contract.methods
        .defaultFeeRatio()
        .call({ from: wallet?.address });
      let v2 = await contract.methods
        .defaultRoyaltyRatio()
        .call({ from: wallet?.address });
      ...
      try {
        ...
        await contract.methods
          .createSale(
            ...
            window.web3.utils.toBN(info.tokenId),
            window.web3.utils.toBN(info.payment),
            window.web3.utils.toBN(info.copy),
            window.web3.utils.toBN(info.method),
            window.web3.utils.toBN(info.duration),
            window.web3.utils.toBN(info.basePrice),
            window.web3.utils.toBN(Math.floor(info.fee)),
            window.web3.utils.toBN(Math.floor(info.royalty))
          )
          .send({ from: wallet?.address })
          .on("receipt", (receipt) => {
            tx = receipt;
    ...
    async (info) => {
      ...
      try {
        ...
        if (info.payment === 0) {
          ...
          let value = BigNumber(`1e${py.decimal}`)
            .times(info.price)
            .times(info.copy);
          ...
          await contract.methods
            .buy(window.web3.utils.toBN(info.saleId))
            .send({
              from: wallet?.address,
              value: window.web3.utils.toBN(value),
            })
            .on("receipt", (receipt) => {
              tx = receipt;
  ...
  const removeSale = useCallback(
    async (saleId) => {
      ...
      try {
        ...
        await contract.methods
          .removeSale(window.web3.utils.toBN(saleId))
          .send({ from: wallet?.address })
          .on("receipt", (receipt) => {
            tx = receipt;
    ...
    async (info) => {
      ...
      try {
        ...
        if (info.payment === 0) {
          ...
          let priceBigNumber = BigNumber(`1e${py.decimal}`)
            .times(info.price)
            .toString();

          await contract.methods
            .placeBid(
              window.web3.utils.toBN(info.saleId),
              window.web3.utils.toBN(priceBigNumber)
            )
            .send({
              from: wallet?.address,
              value: window.web3.utils.toBN(value),
  ...
  const removeOffer = useCallback(
    async (info) => {
      ...
      try {
        ...
        await contract.methods
          .removeOffer(window.web3.utils.toBN(info.saleId))
          .send({ from: wallet?.address })
          .on("receipt", (receipt) => {
            tx = receipt;
  ...
  const acceptOffer = useCallback(
    async (info) => {
      ...
      try {
        ...
        await contract.methods
          .acceptOffer(window.web3.utils.toBN(info.saleId))
          .send({ from: wallet?.address })
          .on("receipt", (receipt) => {
            tx = receipt;
            toastSuccess(auth?.loggedUserName, "Removed an offer successfully");
  ...
  const approveToken = useCallback(
    async (info) => {
      ...
      try {
        ...
        let vv = BigNumber(`1e${py.decimal}`)
          .times(info.price)
          .times(info.copy);
        ...
        tx = await tokenContract.methods
          .approve(factoryAddress, window.web3.utils.toBN(vv))
          .send({ from: wallet?.address });
      } catch (err) {
        console.log(err);
      }
  ...
  const tokenAmountWithoutDecimal = useCallback(
    (paymentIndex, amount) => {
      let py = null;
      try {
      ...
      if (py === null || py === undefined) return "0";

      return BigNumber(amount)
        .times(BigNumber(`1e${py.decimal}`))
        .toString();
    },
    [payment]
  );

  const tokenAmountWithDecimal = useCallback(
    (paymentIndex, amount) => {
      let py = null;
      try {
        py = payment.find((item) => item.id === paymentIndex);
      ...
      if (py === null || py === undefined) return BigNumber(0);
  ...
  const getPaymentName = useCallback(
    (paymentIndex) => {
      let py = null;
      try {
        py = payment.find((item) => item.id === paymentIndex);
      ...
      if (py === null || py === undefined) return "unknown";
